...
- **Komunikacja ze sterownikiem = `ControllerTransport`** ([`lib/data/transport/`](lib/data/transport/)). UI i providery rozmawiają przez `activeTransportProvider` (zwraca `ControllerTransport`), NIGDY przez konkretną implementację. `sendCommand(target, payload)` przyjmuje device-qualified `target` `"<id>/<target>"` i payload jako String — nie używaj `MqttClientPayloadBuilder` w UI. Wspólna logika MQTT jest w `MqttTransportBase`; `CloudMqttTransport`/`LocalMqttTransport` różnią się tylko TLS+endpointem. **Faza 4: telemetria w grupach** — transport subskrybuje `001/state/+` i merguje cząstki (`sensors`/`devices`/`system`) w jeden `SensorModel` (akumulator `_stateAcc`). Nowe pola statusowe (`heater1/2`, `fan1/2`) w `SensorModel` przez `OutputStatusModel`. Każde nowe pole telemetrii = pole w `SensorModel` + `dart run build_runner build --delete-conflicting-outputs`. **Wyboru kanału NIE robisz ręcznie** — `TransportManager` (Notifier za `activeTransportProvider`) sam preferuje lokalny (mDNS w LAN), fallback chmura, re-eval przy: zmianie sieci, powrocie z tła (`AppLifecycleState.resumed`), `avail=false` z transportu. Rodzaj kanału: `activeTransportKindProvider`. **Android locki** zaimplementowane w [`android/app/src/main/kotlin/.../MainActivity.kt`](android/app/src/main/kotlin/com/example/sterownik_akwarium/MainActivity.kt): `WifiLock(WIFI_MODE_FULL_HIGH_PERF)` (utrzymuje WiFi przy uśpieniu) + `MulticastLock` (mDNS discovery) — acquirowane w `onResume`, zwalniane w `onDestroy`. Wymagane uprawnienia: `WAKE_LOCK`, `ACCESS/CHANGE_WIFI_STATE`, `CHANGE_WIFI_MULTICAST_STATE` — wszystkie w manifeście.
